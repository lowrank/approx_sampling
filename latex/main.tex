...
\begin{equation}
  f(x) = \sin(2\pi x) + \sum_{k=1}^5 a_k e^{-(x-\mu_k)^2/(2\sigma_k^2)},
\end{equation}

with five narrow Gaussian spikes of alternating signs placed at
$x\in\{0.15,0.32,0.50,0.68,0.84\}$ (FWHM $\approx 1.5$--$2.5\%$ of
$[0,1]$).  At budget 128 a uniform grid places only 1--2 points per spike.

We compare two architectures: \textbf{MMNN} (rank 15, width 1000) and a
standard \textbf{MLP} ($[32,32,32,32]$, $\tanh$).  All 12 algorithms are
run for 12\,000 effective gradient steps with learning rate $1.5\times10^{-3}$.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/spike_bg_MMNN.pdf}
  \caption{MMNN (rank 15, width 1000) — pointwise mean residual $\pm 1\sigma$
           over 20 random seeds.  Shaded bands show run-to-run variability;
           averaged sampling-point histograms are in blue.  Learnable methods
           (policy, importance sampling, diffusion, adversarial) outperform
           fixed grids on average but with higher variance.}
  \label{fig:spike_bg_mmnn}
\end{figure}
