...
eliminates the aliasing.  The ranking reverses:
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/osc_MLP.pdf}
  \caption{Residuals on the oscillatory function with MLP.  Adaptive
           residual now leads by $6.5\times$ over uniform sampling,
           confirming that the MMNN's $\sin$ random features caused
           aliasing that masked the benefits of adaptive sampling.}
  \label{fig:osc_mlp}
\end{figure}

The adaptive residual method ($L^2=0.082$) beats uniform sampling ($0.779$)
by nearly $10\times$, demonstrating that when the approximator can properly
represent the target function, concentrating samples in high-error regions
provides a decisive advantage.  The DeepMMNN's frozen $\sin$ random features
create spectral aliasing that prevents the model from benefiting from
adaptive sampling, even when points are correctly placed.
