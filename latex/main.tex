...
The same pattern holds: fixed-grid methods (QMC Halton 0.073, uniform 0.083)
outperform the learnable methods (adversarial 0.203, diffusion 0.315) by
2--4$\times$.  The oscillatory function is smooth everywhere, so the
generative methods gain no advantage from concentrating samples, while the
fixed grids provide guaranteed Nyquist coverage.
