...
points.  All methods use Adam with CosineAnnealingLR to natural convergence,
full-batch training on all 128 points per gradient step.

\begin{table}[H]
  \centering
  \caption{Mean $L^2$ error on the hat+bg function (MLP 128$\times$4, budget 128,
           full-batch training to convergence).}
  \label{tab:hat_bg}
  \small
  \begin{tabular}{@{}lrlr@{}}
    \toprule
    ...
    \midrule
    qmc\_halton      & 0.079 & policy           & 0.346 \\
    uniform          & 0.082 & normalizing\_flow & 0.373 \\
    qmc\_sobol       & 0.105 & trainable\_pts    & 0.486 \\
    mdn              & 0.130 & neural\_process   & 0.649 \\
    chebyshev        & 0.138 & density\_network  & 1.857 \\
    diffusion        & 0.156 &                  & \\
    importance\_sampling & 0.211 &                & \\
    adversarial      & 0.250 &                  & \\
    adaptive\_residual & 0.291 &                 & \\
    \bottomrule
  \end{tabular}
\end{table}

Fixed-grid methods (QMC Halton 0.079, uniform 0.082) lead, with MDN
(0.130) as the best learnable method.  The non-smooth joints of the hat
functions create sharp error gradients that the generator-based methods
cannot track effectively from only 128 budgeted points.  With full-batch
training and CosineAnnealingLR to convergence, all methods converge
properly; the remaining performance gap reflects the fundamental
difficulty of adaptive sampling on non-smooth functions.

Figure~\ref{fig:spike_bg_mlp_g1}--\ref{fig:spike_bg_mlp_g2} show the
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/spike_bg_MLP_g1.pdf}
  \caption{MLP $[128,128,128,128]$ — residuals and sampling histograms,
           group 1/2 (best 7 algorithms).  Orange histograms show where
           points were allocated.}
  \label{fig:spike_bg_mlp_g1}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/spike_bg_MLP_g2.pdf}
  \caption{MLP — group 2/2.  Learnable methods over-concentrate at the
           hat kinks while neglecting the smooth background.}
  \label{fig:spike_bg_mlp_g2}
\end{figure}
...
\begin{table}[H]
  \centering
  \caption{$L^2$ errors (MLP 128$\times$4, budget 128, full-batch training).}
  \label{tab:nonper}
  \small
  \begin{tabular}{@{}lr@{}}
    \toprule
...
With full-batch training (all 128 points per gradient step, CosineAnnealingLR
to natural convergence), the fixed-grid methods achieve excellent accuracy.
Chebyshev nodes lead ($1.5\times10^{-4}$), followed by QMC Halton
($7.3\times10^{-4}$) and uniform ($9.5\times10^{-4}$).  The adversarial
generator method ($7.9\times10^{-3}$) trails by $\sim 50\times$, confirming
that smooth non-uniform functions benefit most from guaranteed Nyquist
coverage rather than adaptive sample allocation.
