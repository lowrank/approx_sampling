...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/spike_bg_residuals.pdf}
  \caption{Pointwise residuals $f(x) - u_\theta(x)$ for each algorithm at
           budget 192, sorted by $L^2$ error (best at top).  The two spike
           regions at $x=0.3$ and $x=0.7$ are clearly visible in the
           residuals of fixed-grid methods; learnable methods suppress them
           almost completely.}
