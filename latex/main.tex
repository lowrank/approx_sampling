...
To illustrate the advantage of adaptive and learnable sampling on a concrete
example, we consider
...
shown in Figure~\ref{fig:spike_bg}.  It combines a smooth oscillatory
background with two narrow Gaussian spikes (FWHM $\approx 2.4\%$ and $2.0\%$
of $[0,1]$).  A uniform grid of 64--192 points places only 1--2 points within
each spike.
...
\begin{figure}[H]
  ...
  \caption{Pointwise residuals $f(x) - u_\theta(x)$ for all 15 algorithms at
           budget 192, grouped by performance (best at top).  The two spike
           regions at $x=0.3$ and $x=0.7$ dominate the residuals of
           fixed-grid methods; learnable methods suppress them nearly
           completely.}
  \label{fig:spike_bg_residuals}
\end{figure}

At budget 192 the best algorithms are MDN ($2.96\times10^{-3}$), policy
($3.03\times10^{-3}$), neural process ($3.26\times10^{-3}$), adversarial
($3.36\times10^{-3}$), and diffusion ($3.60\times10^{-3}$).  Uniform
($8.6\times10^{-2}$) and Chebyshev ($8.2\times10^{-2}$) are $\sim 25\times$
